...
#include "ComponentUUVManipulatorOpcUaBackendPortFactory.hh"
...
static ComponentUUVManipulatorOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentUUVManipulatorOpcUaBackendPortFactory::ComponentUUVManipulatorOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentUUVManipulator::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentUUVManipulatorOpcUaBackendPortFactory::~ComponentUUVManipulatorOpcUaBackendPortFactory()
{  }

void ComponentUUVManipulatorOpcUaBackendPortFactory::initialize(ComponentUUVManipulator *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentUUVManipulatorOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentUUVManipulatorOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
Smart::ISendServerPattern<CommBasicObjects::CommNavigationVelocity> * ComponentUUVManipulatorOpcUaBackendPortFactory::createNavigationVelocityServiceIn(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::SendServer<CommBasicObjects::CommNavigationVelocity>(componentImpl, serviceName);
}


int ComponentUUVManipulatorOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentUUVManipulatorOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentUUVManipulatorOpcUaBackendPortFactory::destroy()
{
