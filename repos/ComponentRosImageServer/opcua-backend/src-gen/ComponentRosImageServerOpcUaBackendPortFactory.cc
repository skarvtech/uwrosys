...
#include "ComponentRosImageServerOpcUaBackendPortFactory.hh"
...
static ComponentRosImageServerOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentRosImageServerOpcUaBackendPortFactory::ComponentRosImageServerOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentRosImageServer::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentRosImageServerOpcUaBackendPortFactory::~ComponentRosImageServerOpcUaBackendPortFactory()
{  }

void ComponentRosImageServerOpcUaBackendPortFactory::initialize(ComponentRosImageServer *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentRosImageServerOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentRosImageServerOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
Smart::IPushServerPattern<DomainVision::CommVideoImage> * ComponentRosImageServerOpcUaBackendPortFactory::createRGBImagePushServiceOut(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::PushServer<DomainVision::CommVideoImage>(componentImpl, serviceName);
}


int ComponentRosImageServerOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentRosImageServerOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentRosImageServerOpcUaBackendPortFactory::destroy()
{
