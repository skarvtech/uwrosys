...
pre-start)
  ...
  xterm -title 'ROS Core Terminal' -e bash -c "roscore || bash; sleep 5" &
  xterm -title 'ROSJoy Terminal' -e bash -c 'sleep 2 && rosparam set joy_node/dev "/dev/input/js2" && rosrun joy joy_node || bash; sleep 5' &
;;
