...
#include "ComponentUUVSonarOpcUaBackendPortFactory.hh"
...
static ComponentUUVSonarOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentUUVSonarOpcUaBackendPortFactory::ComponentUUVSonarOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentUUVSonar::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentUUVSonarOpcUaBackendPortFactory::~ComponentUUVSonarOpcUaBackendPortFactory()
{  }

void ComponentUUVSonarOpcUaBackendPortFactory::initialize(ComponentUUVSonar *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentUUVSonarOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentUUVSonarOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IPushClientPattern<CommBasicObjects::CommBaseState> * ComponentUUVSonarOpcUaBackendPortFactory::createBaseStateServiceIn()
{
	return new SeRoNet::OPCUA::Client::PushClient<CommBasicObjects::CommBaseState>(componentImpl);
}


Smart::IPushServerPattern<CommBasicObjects::CommMobileLaserScan> * ComponentUUVSonarOpcUaBackendPortFactory::createLaserServiceOut(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::PushServer<CommBasicObjects::CommMobileLaserScan>(componentImpl, serviceName);
}


int ComponentUUVSonarOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentUUVSonarOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentUUVSonarOpcUaBackendPortFactory::destroy()
{
