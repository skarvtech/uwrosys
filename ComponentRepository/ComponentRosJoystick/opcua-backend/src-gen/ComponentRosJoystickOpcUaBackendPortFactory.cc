...
#include "ComponentRosJoystickOpcUaBackendPortFactory.hh"
...
static ComponentRosJoystickOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentRosJoystickOpcUaBackendPortFactory::ComponentRosJoystickOpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentRosJoystick::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentRosJoystickOpcUaBackendPortFactory::~ComponentRosJoystickOpcUaBackendPortFactory()
{  }

void ComponentRosJoystickOpcUaBackendPortFactory::initialize(ComponentRosJoystick *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentRosJoystickOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentRosJoystickOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
Smart::IPushServerPattern<CommBasicObjects::CommJoystick> * ComponentRosJoystickOpcUaBackendPortFactory::createJoystickServiceOut(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::PushServer<CommBasicObjects::CommJoystick>(componentImpl, serviceName);
}


int ComponentRosJoystickOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentRosJoystickOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentRosJoystickOpcUaBackendPortFactory::destroy()
{
